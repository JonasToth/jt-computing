...
TEST_CASE("MiddleLong String > 56 && < 64", "") {
  auto data = string{"oalksdjf oiajsdoifjqlwejroi "
...
ducimus non amet assumenda.)"};
  ...
  for (usize chunkSize = 1; chunkSize < 367; ++chunkSize) {
    ...
    const auto sv           = string_view{lorem};
...
TEST_CASE("Sha256 throws when digested, but not reset", "") {
  ...
  REQUIRE_THROWS_AS(s.process("More Content"sv), runtime_error);
}
...
TEST_CASE("Hash single-linked list of characters", "") {
  ...
  const auto charList = forward_list<char>{begin(s1), end(s1)};
  s.process(charList);
  REQUIRE(s.digest() == expectedHash);
}

TEST_CASE("Hash istream-view", "") {
  auto iss = istringstream{"Stream Class Iterator hashing works"};
  auto s   = Sha256Sum{};
...
TEST_CASE("Hash istream range", "") {
  auto iss = istringstream{"Temporary File Content\r\n"};

  auto s   = Sha256Sum{};
  s.process(ranges::subrange{istreambuf_iterator<char>{iss},
...
TEST_CASE("Hash istream iterator-interface", "") {
  ...
  auto s   = Sha256Sum{};
  s.process(istreambuf_iterator<char>{iss}, istreambuf_iterator<char>{});
...
TEST_CASE("Benchmark for Sha256", "[.]") {
  ...
  const string refStr = generateToHash();

...
module jt.Math:TestModularArithmetic;
...
TEST_CASE("Example RSA Fast", "") {
  ...
  auto n_modulus          = fromHexString<NaturalN>(modulus);
  auto n_public_exponent  = fromHexString<NaturalN>(public_exponent);
  auto n_private_exponent = fromHexString<NaturalN>(private_exponent);
  auto n_prime1           = fromHexString<NaturalN>(prime1);
  auto n_prime2           = fromHexString<NaturalN>(prime2);
  auto n_exponent1        = fromHexString<NaturalN>(exponent1);
  auto n_exponent2        = fromHexString<NaturalN>(exponent2);
  auto n_coefficient      = fromHexString<NaturalN>(coefficient);
  auto n_hash             = fromHexString<NaturalN>(hash_document);

  SECTION("Good Values") {
    REQUIRE(n_public_exponent == 65537_U);
    REQUIRE(toString(n_hash) == "7775362756942223229477270152222009939428593292"
                                "5889711304898348330393612505397");
    REQUIRE(toString(n_private_exponent) ==
            "132687760831157997173534550746869794686579580050563210254007193414"
  ...
  SECTION("Signing") {
    ...
    const auto opensslSignature =
        ...
        "2962c28a16029ee86747901eac0046fdad38674d7867128c69adc46de369391fb875c0"
        "de43141215b4bb2bdf968bcf555169a08d92982bc77acd"s;
    const auto opensslSignatureInt = fromHexString<NaturalN>(opensslSignature);
    REQUIRE(opensslSignatureInt != 0_U);
